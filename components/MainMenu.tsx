...
import { Truck, FileBarChart, History, ChevronRight, BarChart3, Image as ImageIcon, RefreshCw } from 'lucide-react';

interface MainMenuProps {
  onSelectView: (view: 'llegada' | 'informe' | 'memoria' | 'ddd' | 'galeria' | 'cambioTurno') => void;
}
...
const MainMenu: React.FC<MainMenuProps> = ({ onSelectView }) => {
  return (
    <div className="h-screen w-full flex flex-col items-center justify-center bg-gradient-to-br from-calido to-levanda p-6 gap-12 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6 max-w-[98rem] w-full px-4">
        <button
